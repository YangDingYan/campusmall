<template>
  <el-container class="GoodsDetail">
    <el-main class="content">
      <div class="goodsInfo">
        ...
        <div class="infoRight">
          ...
          <div class="infoBox">
            <span>规格：</span>
            <span style="width: 200px;">{{ '页数: '+goodsPages+' ,   单价: '+goodsUnitPrice }}</span>
          </div>
          <div class="infoBox">
            <span>数量：</span>
            <el-input-number v-model="num" :min="1" :max="60" label="描述文字"></el-input-number>
          </div>
      ...
      <div style="margin: 0 auto;width: 1200px;">
        <section class="msgBox leftContainer">
              ...
              <ul class="commentList">
                <li v-for="(item,index) in commentList" :key="'comment'+index">
                  ...
                  <div class="commentInfo">
                    <div class="starList">
                      <i class="iconfont icon-collection_fill" v-for="(star,index) in (item.score/20)" :key="item.id+''+index" />
                    </div>
          ...
          <div class="msgBody" v-else>
            <div class="inputBox">
              ...
              <div v-else class="banAsk" @click="navTo('/login')">请先登录</div>
            </div>
            <ul class="msgList">
        ...
        <section class="typeGoods rightContainer">
          ...
          <ul class="list">
            <GoodsItem v-for="(item,index) in filterList" :key="+item.id" :id="item.id" :img="item.img" :name="item.name"
              :price="item.price" />
          </ul>
        </section>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      goodsPrice() {
        return (this.num * this.goodsUnitPrice * this.goodsPages).toFixed(2)
      },
      filterList() {
    ...
    methods: {
      ...
      getGoodsInfo(id) {
        ...
        res
          .then((data) => {
            ...
            this.goodsDesc = data.description;
            this.goodsUnitPrice = data.unitPrice;
            this.goodsPages = data.pages;
      ...
      addToCart() {
        ...
        let userId = getLocalItem('userId')
        const res = addOrder({
          id: 0,
          userid: userId,
          goodsId: this.id,
      ...
      buy() {
        ...
        let loginId = getLocalItem('userId')
        const res = addOrder({
          id: 0,
          userid: loginId,
          goodsId: this.id,
